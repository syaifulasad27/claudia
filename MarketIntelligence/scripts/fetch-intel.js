...
async function fetchForexFactory() {
  const res = await fetch(FF_URL, { headers: { 'User-Agent': 'MarketIntelligence/0.1' } });
  ...
  return list.filter(Boolean).map((e) => ({
    ...
    timestamp_utc: inferUtcDate(e.date, e.time),
    source: 'forexfactory',
    type: 'economic',
    title: e.title ?? 'Untitled event',
    currency: e.currency ?? null,
    impact_raw: e.impact ?? e.impact_title ?? null,
...
async function fetchYahoo() {
  const feed = await rss.parseURL(YAHOO_URL);
  return (feed.items || []).map((item, i) => {
    const text = `${item.title || ''} ${item.contentSnippet || ''}`.toLowerCase();
    return {
      id: `yahoo-${item.guid || item.link || i}`,
...
async function maybeBraveFallback() {
  const key = process.env.BRAVE_API_KEY;
  if (!useBrave || !key) return { used: false, items: [], reason: 'disabled-or-missing-key' };

  const now = Date.now();
  const state = await readJson(BRAVE_STATE, { lastRequestMs: 0 });
  const oneHour = 60 * 60 * 1000;
  if (now - (state.lastRequestMs || 0) < oneHour) {
    return { used: false, items: [], reason: 'hourly-cap-reached' };
  }
  ...
  const res = await fetch(url, {
    headers: {
      ...
      'User-Agent': 'MarketIntelligence/0.1'
    }
  });

  if (!res.ok) {
    return { used: false, items: [], reason: `brave-http-${res.status}` };
  }
...
function inferUtcDate(date, time) {
  if (!date) return null;
  const composed = `${date} ${time || '00:00am'} UTC`;
...
function mapAssetsFromText(text = '') {
  ...
  if (assets.length === 0) assets.push('XAUUSD', 'NASDAQ');
  return [...new Set(assets)];
}
...
async function main() {
  ...
  const [ff, yahoo, brave] = await Promise.allSettled([
    fetchForexFactory(),
    fetchYahoo(),
    maybeBraveFallback()
  ]);
  ...
  const payload = {
    ...
    sources: {
      ...
      yahoo: yahoo.status === 'fulfilled' ? 'ok' : `error: ${yahoo.reason?.message || yahoo.reason}`,
      brave: brave.status === 'fulfilled' ? brave.value.reason : `error: ${brave.reason?.message || brave.reason}`
    },
    items: [
      ...(ff.status === 'fulfilled' ? ff.value : []),
      ...(yahoo.status === 'fulfilled' ? yahoo.value : []),
      ...(brave.status === 'fulfilled' ? brave.value.items : [])
    ]
  };
